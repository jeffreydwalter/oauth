...
func (s *OAuthBearerServer) UserCredentials(w http.ResponseWriter, r *http.Request) {
	...
	code, resp := s.generateTokenResponse(grantType, username, password, refreshToken, scope, "", "", r)
...
func (s *OAuthBearerServer) ClientCredentials(w http.ResponseWriter, r *http.Request) {
	...
	// grant_type refresh_token
	refreshToken := r.FormValue("refresh_token")
	code, resp := s.generateTokenResponse(grantType, clientId, clientSecret, refreshToken, scope, "", "", r)
	render.JSON(w, code, resp)
}
...
func (s *OAuthBearerServer) AuthorizationCode(w http.ResponseWriter, r *http.Request) {
	...
	status, resp := s.generateTokenResponse(grantType, clientId, clientSecret, "", scope, code, redirectURI, r)
	render.JSON(w, status, resp)
}
